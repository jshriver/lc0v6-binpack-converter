...
//! Lookup table mapping lc0 policy indices to move strings.
//! Source: kMoveStrs array in lc0/src/neural/encoder.cc

pub const MOVE_STRS: [&str; 1858] = [
...
/// Convert a policy index to its UCI move string (e.g. 420 -> "e2e4").
/// Returns "<invalid>" if the index is out of range.
pub fn idx_to_move(idx: u16) -> &'static str {
    MOVE_STRS.get(idx as usize).copied().unwrap_or("<invalid>")
}